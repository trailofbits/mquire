...
use crate::memory::{RawVirtualAddress, Readable, VirtualAddress};
use crate::sys::{Architecture, Endianness, Error as SystemError, Result as SystemResult};
...
impl<'a> VirtualMemoryReader<'a> {
    ...
    pub fn read_vaddr(&self, virtual_address: VirtualAddress) -> SystemResult<VirtualAddress> {
        let raw_virtual_address = match virtual_address.get() {
            RawVirtualAddress::U32(_) => RawVirtualAddress::U32(self.read_u32(virtual_address)?),
            RawVirtualAddress::U64(_) => RawVirtualAddress::U64(self.read_u64(virtual_address)?),
        };
