...
use crate::operating_system::linux::{BtfparseReadableAdapter, VirtualStruct};
use crate::sys::entities::{File, OSVersion, SystemInformation, Task};
use crate::sys::{
    Architecture, Endianness, Error as SystemError, ErrorKind as SystemErrorKind, OperatingSystem,
    OperatingSystemData, Result as SystemResult, VirtualMemoryReader,
};
...
/// Private state data for the OperatingSystem implementation
struct LinuxOperatingSystemData {
...
impl OperatingSystemData for LinuxOperatingSystemData {
    /// Converts the `OperatingSystemData` object into a `dyn Any` object (used for downcasting)
    fn as_any(&self) -> &dyn std::any::Any {
        self as &dyn std::any::Any
    }

    /// Converts the `OperatingSystemData` object into a mutable `dyn Any` object (used for downcasting)
...
impl LinuxOperatingSystem {
    ...
    fn get_kernel_type_info(readable: &dyn Readable) -> SystemResult<TypeInformation> {
        ...
        for chunk_index in 0..chunk_count {
                ...
                })
            {
                ...
                if type_section_size >= MIN_BTF_TYPE_SECTION_SIZE {
                    ...
                    let readable_adapter = BtfparseReadableAdapter::new(readable, btf_offset.get());
        ...
        Err(SystemError::new(
            SystemErrorKind::OperatingSystemInitializationFailed,
    ...
    ) -> SystemResult<(PhysicalAddress, RawVirtualAddress)> {
        let task_struct_tid = kernel_type_info
            .id_of("task_struct")
                ...
                .position(|window| window == SWAPPER_PROCESS_COMM.as_bytes())
            {
                ...
                let swapper_struct_raw_vaddr =
                    if swapper_struct_parent_field == swapper_struct_real_parent_field {
                        RawVirtualAddress::U64(swapper_struct_parent_field)
                    } else {
                        continue;
                    };
    ...
    fn get_task_snapshot(
        ...
        let mut page_table = virtual_address.page_table();
        ...
        if !mm_struct.virtual_address().is_null() {
            ...
            if let Ok(physical_addr_range) = architecture.translate_virtual_address(readable, pgd) {
                ...
                let arg_start_vaddr = VirtualAddress::new(
                    page_table,
                    RawVirtualAddress::U64(mm_struct.traverse("arg_start")?.read_u64()?),
                );

                let arg_end_vaddr = VirtualAddress::new(
                    page_table,
                    RawVirtualAddress::U64(mm_struct.traverse("arg_end")?.read_u64()?),
                );

                let arg_size = arg_end_vaddr.sub(arg_start_vaddr)?;
                ...
                let env_start_vaddr = VirtualAddress::new(
                    page_table,
                    RawVirtualAddress::U64(mm_struct.traverse("env_start")?.read_u64()?),
                );

                let env_end_vaddr = VirtualAddress::new(
                    page_table,
                    RawVirtualAddress::U64(mm_struct.traverse("env_end")?.read_u64()?),
                );
...
impl OperatingSystem for LinuxOperatingSystem {
    ...
    fn initialize(
        ...
        let kernel_type_info = Self::get_kernel_type_info(readable)?;
        let swapper_task_struct_vaddr =
            Self::get_swapper_struct_virtual_addr(readable, architecture, &kernel_type_info)?;

        let operating_system_data = Arc::new(LinuxOperatingSystemData {
            kernel_type_info,
            swapper_task_struct_vaddr,
        });

        Ok(operating_system_data)
    }
    ...
    fn get_os_version(
        ...
        let linux_data = operating_system_data
            .as_any()
            .downcast_ref::<LinuxOperatingSystemData>()
            .ok_or(SystemError::new(
                SystemErrorKind::OperatingSystemInitializationFailed,
                "Failed to downcast operating system data to LinuxOperatingSystemData",
            ))?;

        Self::get_os_version(
            readable,
            architecture,
            &linux_data.kernel_type_info,
            linux_data.swapper_task_struct_vaddr,
        )
    }
    ...
    fn get_task_list(
        ...
        let mut next_virtual_address_queue = vec![linux_data.swapper_task_struct_vaddr];
        ...
        while !next_virtual_address_queue.is_empty() {
            ...
            for virtual_address in virtual_address_queue {
                let physical_address =
                    match architecture.translate_virtual_address(readable, virtual_address) {
                ...
                if visited_physical_address_list.contains(&physical_address.get()) {
                    continue;
                }

                visited_physical_address_list.push(physical_address.get());
    ...
    fn get_task_open_file_list(
        ...
        for task in self.get_task_list(readable, architecture, operating_system_data)? {
            let vmem_reader = VirtualMemoryReader::new(readable, architecture);
            let task_struct = VirtualStruct::from_name(
                &vmem_reader,
                &linux_data.kernel_type_info,
...
impl Default for LinuxOperatingSystem {
    /// Returns a default instance of the LinuxOperatingSystem
    fn default() -> Self {
        LinuxOperatingSystem::new()
    }
}

impl From<BtfparseError> for SystemError {
