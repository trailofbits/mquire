...
| [`130_process_ptrace_flags.sql`](linux/common/130_process_ptrace_flags.sql) | `process_ptrace_flags` | Quick-peek ptrace overview: one row per process, the set `PT_*` flags rendered as a compact space-separated list (`''` when the task is not being traced) plus the raw `task_struct::ptrace` value |
